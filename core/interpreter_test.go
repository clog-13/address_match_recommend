...
var (
	addrs = []string{
		...
		"辽宁沈阳于洪区沈阳市辽中县县城虹桥商厦西侧三单元外跨楼梯3-2-23-",
		"山东济宁任城区金宇路【杨柳国际新城K8栋3单元1302】(杨柳国际新城·丽宫附近)",
	}
)

func TestAddressInterpreter_Interpret(t *testing.T) {
	interpreter := NewAddressInterpreter(NewAddressPersister())
	addr := &models.Address{}
	for _, v := range addrs {
		addr.AddressText = v
		interpreter.Interpret(addr)
		fmt.Println(addr.Province)
		//fmt.Println(addr.Province.Name + "," + addr.City.Name + "," + addr.District.Name + "," +
		//	addr.Street.Name + "," + addr.AddressText + "," + addr.BuildingNum)
	}
}
